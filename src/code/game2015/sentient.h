...
extern Event EV_Sentient_HurtFlame; //### added for flamethrower
...
protected:
   ...
   virtual void      WeaponKnockedFromHands();
...
public:
   ...
   //### added for ropes
   EntityPtr         rope_grabbed;
   float             rope_debounce_time;
   qboolean          ropesound;
   //### added for toggling of auto weapon switching
   int               autoweaponswitch;
   int               weaponoverride;
   //###
   ...
   virtual void      NonAutoChangeWeapon(Weapon *weapon); // added for toggling of auto weapon switching
   virtual int       IsHoldingAttack();

   // warning: hoverbike happyness below
   virtual HoverbikePtr IsOnBike();
   virtual HoverbikePtr GetHoverbike();
   virtual void         SetHoverbike(Hoverbike *bike);
   virtual int          IsFiringMissile();
   virtual int          BikeIsTurboing();
   virtual void         SetBikeAnim(str bikeanim);
   virtual float        WishedBikeYaw();
   virtual float        BikeAimPitch();
   //###
