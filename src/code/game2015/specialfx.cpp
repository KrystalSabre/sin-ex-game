...
void ChangeMusic(const char *current, const char *fallback, qboolean force, float duration)
{
   int      j;
   edict_t	*other;
   ...
   if(current || fallback)
   {
      for(j = 1; j <= game.maxclients; j++)
      {
         ...
         if(other->inuse && other->client)
         {
            ...
            client->ChangeMusic(current, fallback, force, duration);
         }
      }
   }
}
