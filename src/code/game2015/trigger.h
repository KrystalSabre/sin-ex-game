...
public:
   ...
   //###
   virtual void ToggleState(Event *ev);
   virtual void Archive(Archiver &arc)   override;
   virtual void Unarchive(Archiver &arc) override;
   //###
};
...
//###
inline EXPORT_FROM_DLL void TriggerExit::Archive(Archiver &arc)
{
   Trigger::Archive(arc);

   arc.WriteBoolean(togglestate);
}

inline EXPORT_FROM_DLL void TriggerExit::Unarchive(Archiver &arc)
{
   Trigger::Unarchive(arc);

   arc.ReadBoolean(&togglestate);
}
//###
...
protected:
   ...
   float    particletime;
   int      particlesize;
   int      particlegrowrate;
   str      spawntarget;
   int      particlespeed;
   int      particlespread;
   Vector   particleaccel;
   float    attenuation;
   //###
...
public:
   ...
   void         SpawnToggleParticles(Event *ev); //###
...
inline EXPORT_FROM_DLL void TriggerParticles::Archive(Archiver &arc)
{
   ...
   arc.WriteInteger(count);
   //###
   arc.WriteFloat(particletime);
   arc.WriteInteger(particlesize);
   arc.WriteInteger(particlegrowrate);
   arc.WriteString(spawntarget);
   arc.WriteInteger(particlespeed);
   arc.WriteInteger(particlespread);
   arc.WriteVector(particleaccel);
   arc.WriteFloat(attenuation);
   //###
}
...
inline EXPORT_FROM_DLL void TriggerParticles::Unarchive(Archiver &arc)
{
   ...
   arc.ReadInteger(&count);
   //###
   arc.ReadFloat(&particletime);
   arc.ReadInteger(&particlesize);
   arc.ReadInteger(&particlegrowrate);
   arc.ReadString(&spawntarget);
   arc.ReadInteger(&particlespeed);
   arc.ReadInteger(&particlespread);
   arc.ReadVector(&particleaccel);
   arc.ReadFloat(&attenuation);
   //###
}
...
//### added 2015 stuff
class EXPORT_FROM_DLL TriggeredHurt : public TriggerUse
{
protected:
   float    damage;
   float    hurttime;
...
inline EXPORT_FROM_DLL void TriggeredHurt::Archive(Archiver &arc)
{
   TriggerUse::Archive(arc);

   arc.WriteFloat(damage);
   arc.WriteFloat(hurttime);
   arc.WriteFloat(hurtduration);
}

inline EXPORT_FROM_DLL void TriggeredHurt::Unarchive(Archiver &arc)
{
   TriggerUse::Unarchive(arc);

   arc.ReadFloat(&damage);
   arc.ReadFloat(&hurttime);
   arc.ReadFloat(&hurtduration);
}
//###
