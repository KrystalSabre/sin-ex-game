...
void Sentient::AddItem(Item *object)
{
   ...
   //### don't set runes or flags as current item
   //if(object->isSubclassOf(InventoryItem))
   if(object->isSubclassOf<InventoryItem>() &&
      !object->isSubclassOf<CTF_Tech>()     &&
      !object->isSubclassOf<CTF_Flag>())
   {
...
void Sentient::RemoveItem(Item *object)
{
   ...
   if(currentItem == (InventoryItem *)object)
   {
      ...
      currentItem = static_cast<InventoryItem *>(NextItem(nullptr));
      if(!currentItem)
      {
         currentItem = static_cast<InventoryItem *>(PrevItem(nullptr));
      }
   }
}
...
Item *Sentient::FindItem(const char *itemname)
{
   ...
   for(i = 1; i <= num; i++)
   {
      item = static_cast<Item *>(G_GetEntity(inventory.ObjectAt(i)));
...
Weapon *Sentient::BestWeapon(Weapon *ignore)
{
   ...
   for(j = 1; j <= n; j++)
   {
      ...
      if((next != ignore) && next->isSubclassOf<Weapon>() && (static_cast<Weapon *>(next)->Rank() > bestrank) &&
         (static_cast<Weapon *>(next)->HasAmmo()))
      {
         bestweapon = static_cast<Weapon *>(next);
...
Item *Sentient::giveItem(const char * itemname, int amount, int icon_index)
{
   ...
   if(item)
   {
      //### do double max add for ammo
      if(ctf->value && HasItem("CTF_Tech_AmmoVortex") && item->isSubclassOf<Ammo>())
      {
         item->DoubleMaxAdd(amount);
      }
      else
      {
         item->Add( amount );
...
void Sentient::EventTakeAmmo(Event *ev)
{
   ...
   ammo = static_cast<Ammo *>(FindItem(type));
   if(ammo)
   {
      ammo->Remove(amount);
   }
...
void Sentient::EventTakeArmor(Event *ev)
{
   ...
   armor = static_cast<Armor *>(FindItem(type));
   if(armor)
   {
      armor->Remove(amount);
   }
...
EXPORT_FROM_DLL void Sentient::AnimLoop(Event *ev)
{
   ...
   {
      RandomAnimate(currentAnim.c_str(), nullptr);
   }
   else
   {
...
void Sentient::ArmorDamage(Event *ev)
{
   ...
   else if((attacker != this) && (DM_FLAG(DF_MODELTEAMS | DF_SKINTEAMS) || ctf->value))
   //###
   {
   ...
   if(ctf->value)
   {
      ...
      if(attacker->isClient())
      {
         ...
         pl_attacker = static_cast<Player *>(static_cast<Sentient *>(attacker));
         if(pl_attacker->HasItem("CTF_Tech_Double")) // DOUBLE
            damage *= 1.75; // ##! for testing our less powerful runes
         else if(pl_attacker->HasItem("CTF_Tech_DeathQuad")) // QUAD
            damage *= 3.5; // ##! for testing our less powerful runes
         //###
         else if(pl_attacker->HasItem("CTF_Tech_Vampire"))
         {
            // don't drain health from dead or non-player things things
            if(this->deadflag == DEAD_NO && this->isClient())
            {
               Event *ctfev;
               ...
               pl_attacker->ProcessEvent(ctfev);
            }
         }
         //###
      }
      ...
      if(this->isClient())
      {
         ...
         else if(pl_defender->HasItem("CTF_Tech_Empathy"))  // EMPATHY
         {
            ...
            if((attacker != this) && attacker->isClient())
            {
               ...
               ev2->AddInteger(damage * 0.75); // ##! for testing our less powerful runes
               ev2->AddEntity(this);
               ev2->AddEntity(this);
...
Item *Sentient::NextItem(Item *item)
{
   ...
   if(ctf->value)
   {
      for(i = 1; i <= n; i++)
      {
         ...
         if(next_item->isSubclassOf<InventoryItem>() &&
            !next_item->isSubclassOf<CTF_Tech>() &&
            !next_item->isSubclassOf<CTF_Flag>() &&
            item_found)
            return next_item;
...
Item *Sentient::PrevItem(Item *item)
{
   ...
   //### don't select runes or flags as items in CTF
   if(ctf->value)
   {
      for(i = n; i >= 1; i--)
      {
         ...
         if(prev_item->isSubclassOf<InventoryItem>() &&
            !prev_item->isSubclassOf<CTF_Tech>() &&
            !prev_item->isSubclassOf<CTF_Flag>() &&
            item_found)
            return prev_item;
...
EXPORT_FROM_DLL void Sentient::Unarchive(Archiver &arc)
{
   ...
   tempAnimEvent = new Event(arc.ReadEvent()); // haleyjd 20170603: put back in with proper const & copy constructor
...
void Sentient::WritePersistantData(SpawnArgGroup &group)
{
   ...
   for(i = 1; i <= num; i++)
   {
      ...
      if(ent && ent->isSubclassOf<Item>())
      {
         ...
         static_cast<Item *>(ent)->CreateSpawnArgs();
...
void Sentient::RestorePersistantData(SpawnArgGroup &group)
{
   ...
   if(item.length())
   {
      ...
      if(it)
      {
         ...
         currentItem = static_cast<InventoryItem *>(it);
      }
   }
...
void Sentient::DoubleArmor()
{
   ...
   for(i = 1; i <= n; i++)
   {
      auto item = static_cast<Item *>(G_GetEntity(inventory.ObjectAt(i)));
...
void Sentient::NonAutoChangeWeapon(Weapon *weapon)
{
   ...
   if(currentWeapon)
   {
      ...
      if(!Q_strcasecmp(currentWeapon->getClassname(), "Magnum") &&
         !Q_strcasecmp(weapon->getClassname(), "DualMagnum"))
      {
         newWeapon = weapon;
         currentWeapon->PutAway();
      }
...
void Sentient::CheckAmmoCaps()
{
   int num = inventory.NumObjects();
   for(int i = 1; i <= num; i++)
   {
      auto item = static_cast<Item *>(G_GetEntity(inventory.ObjectAt(i)));
      if(item->isSubclassOf<Ammo>())
      {
         // this will make each ammo item
...
int          Sentient::IsHoldingAttack()             { return 0;             }
HoverbikePtr Sentient::IsOnBike()                    { return hoverbike;     }
HoverbikePtr Sentient::GetHoverbike()                { return hoverbike;     }
void         Sentient::SetHoverbike(Hoverbike *bike) { hoverbike = bike;     }
int          Sentient::IsFiringMissile()             { return 0;             }
int          Sentient::BikeIsTurboing()              { return 0;             }
void         Sentient::SetBikeAnim(str bikeanim)     {}
