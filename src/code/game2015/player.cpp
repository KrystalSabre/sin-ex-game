...
void Player::InitMusic(void)
{
   ...
   ChangeMusic(level.default_current_mood.c_str(), level.default_fallback_mood.c_str(), level.default_music_forced, 0);
}
...
void Player::SetCameraEntity(Entity *cameraEnt)
{
   ...
   else
   {
      ...
      if(cameraEnt == thirdpersonCamera)
      {
         ...
         event->AddFloat(vehicle ? 256 : (hoverbike ? 160 : 128));
      ...
      if(ctf->value)
      {
         if(HasItem( "CTF_Tech_Vampire"))
         {
            if((int)health > max_health + CTF_TECH_VAMPIRE_DECAY_HEALTH)
            {
               ...
               if((int)health < max_health + CTF_TECH_VAMPIRE_DECAY_HEALTH)
                  health = max_health + CTF_TECH_VAMPIRE_DECAY_HEALTH;
            }
         }
...
EXPORT_FROM_DLL void Player::UpdateStats()
{
   ...
   //### lap time stats
   if(!ctf->value && level.cp_num) // no lap tracking stuff in CTF, stat #'s used for CTF stuff
   {
      if(player->last_goal_time)
      {
         client->ps.stats[STAT_CURRENTLAP] = (int)((level.time - player->last_goal_time)*10);
      }
      else
      {
      ...
      if(player->client->resp.last_lap_time)
      {
         client->ps.stats[STAT_LASTLAP] = (int)(player->client->resp.last_lap_time*10);
      }
      else
      {
      ...
      if(level.cp_num)
      {
         client->ps.stats[STAT_CPCOUNT] = level.cp_num - player->cp_list.length();
      }
      else
      {
...
EXPORT_FROM_DLL void Player::UpdateMusic()
{
   ...
   else if(action_level < 15 && client->ps.current_music_mood == mood_action && music_current_mood == mood_action)
   {
      music_current_mood = mood_normal;
      music_fallback_mood = mood_normal;
      client->ps.current_music_mood = music_current_mood;
      client->ps.fallback_music_mood = music_fallback_mood;
      music_duration = level.time + 1.2f;
      music_forced = 2;
   }
   ...
   if(action_level > 0)
   {
      if(music_current_mood == mood_action && action_level_decrement >= 0.2f)
         action_level_decrement += 0.01f;
      else
   ...
   if(current)
   {
      ...
      else
      {
         if(level.music_default_duration[current_mood_num] > 0 && duration != -1 || duration > 0)
         {
            ...
            if(!music_duration || !(client->ps.current_music_mood == current_mood_num && level.music_default_duration[current_mood_num] > 0 && music_duration <= level.time + duration))
...
void Player::IncreaseActionLevel(float action_level_increase)
{
   if(music_current_mood == mood_normal && action_level <= 30 && action_level_increase < 8.0f)
      action_level_increase = min(action_level_increase * (2.5 - action_level * 0.05), 8);
