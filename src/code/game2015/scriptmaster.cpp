...
static ScriptVariablePtr GameTime = nullptr;
...
CLASS_DECLARATION(Class, ThreadMarker, nullptr);
...
CLASS_DECLARATION(Listener, ScriptMaster, nullptr);
...
CLASS_DECLARATION(Listener, ScriptThread, nullptr);
...
void ScriptThread::Spawn(Event *ev)
{
   ...
   targetname = G_GetSpawnArg("target", nullptr);
...
void ScriptThread::TriggerFromEntEvent(Event *ev)
{
   ...
   else if(name.length() && name[0] == '*')   // Check for entnum commands
   {
      if(!IsNumeric(&name[1]))
      {
         ScriptError("Expecting numeric value for * command, but found '%s'\n", &name[1]);
