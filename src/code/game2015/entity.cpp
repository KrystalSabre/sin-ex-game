...
Entity::~Entity()
{
   ...
   // SINEX_TODO: should probably apply to base game also (Burger Becky fix for memory leak)
   StopAnimating(); /* Release the animDoneEvent is present */
...
EXPORT_FROM_DLL void Entity::link(void)
{
   gi.linkentity(edict);
   absmin = edict->absmin;
   absmax = edict->absmax;
...
void Entity::StopAnimating(void)
{
...
void Entity::StartAnimating(void)
{
   // start animating
   AnimateFrame();
   animating = true;
